...
<%
	...
	*/
	...
	if(isSave != null){
		...
		//아이디 비밀번호를 쿠키로 응답하고 1주일 동안 유지 되도록 한다.
		...
		//쿠키 경로
		//cook1.setPath("/"); //모든 경로에서 사용할수 있는 쿠키
		//cook2.setPath("/user/"); //user 경로에서만 사용할수 있는 쿠키
		// 설정하지 않으면 현재 경로에서만 사용할수 있음
		
		//HttpServletResponse  객체에 Cookie 객체를 담으면 응답할때 알아서 쿠키가 응답된다.
		...
		//쿠키 유지시간을 0 로 설정해서 응답하면 쿠키가 삭제되는 효과를 낸다 
		cook1.setMaxAge(0);
		cook2.setMaxAge(0);
		response.addCookie(cook1);
		response.addCookie(cook2);
